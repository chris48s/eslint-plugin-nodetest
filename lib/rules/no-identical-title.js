...
function getStringTitle(node) {
  ...
  return evaluateStringNode(firstArg);
}

/**
 * Attempt to statically evaluate an AST node as a string. Handles:
 *
 * - String Literal: 'foo'
 * - Static TemplateLiteral (no expressions): `foo`
 * - BinaryExpression with operator '+' where both sides are static strings
 *
 * Returns null when the node cannot be statically reduced to a string.
 *
 * @param {import("eslint").Rule.Node} node
 * @returns {string | null}
 */
function evaluateStringNode(node) {
  ...
  if (node.type === "TemplateLiteral" && node.expressions.length === 0) {
    ...
    return cooked !== null ? cooked : node.quasis[0].value.raw;
  }

  // Binary '+' of two statically foldable string expressions
  if (node.type === "BinaryExpression" && node.operator === "+") {
    const left = evaluateStringNode(node.left);
    if (left === null) return null;
    const right = evaluateStringNode(node.right);
