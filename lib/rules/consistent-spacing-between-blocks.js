...
function hasBlankLineBetween(sourceCode, prevToken, node) {
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    const nextStart =
      i + 1 < all.length ? all[i + 1].loc.start.line : endLine;
  ...
  create(context) {
    ...
    return {
      ...
      FunctionExpression(node) {
        if (isSuiteCallbackFunction(node) || isSuiteCallbackByReference(node, suiteCallbackIds)) enterSuiteCallback(node);
      },

      "FunctionExpression:exit"(node) {
        if (isSuiteCallbackFunction(node) || isSuiteCallbackByReference(node, suiteCallbackIds)) exitSuiteCallback();
      },
